...
# 2-25
train = filter(marr_rf, date < ymd('2021-04-01
                                   '))
# test =  filter(marr_rf, date >= ymd('2020-01-01'))

model_table = model(train, ann = ETS(total ~ error('A') +
                                       trend('N') +
                                       season('N')))
model_table

fcst = forecast(model_table, h = 6)
fcst %>% autoplot(filter(marr_rf, date > ymd('2019-01-01'))) + labs(title = "ETS(ANN)", y = "Number of weddings", x = NULL) +
  theme_light() + theme(legend.position = "none")
ggsave("../pictures/om_ts_02-025.png", dpi = 300, width = plot_width, height = plot_height, unit = 'cm')


# 2-52
nrow(marr_rf)
marr_rf_fake = mutate(marr_rf, total = total + 1.5 * (1:nrow(marr_rf)))
train = filter(marr_rf_fake, date < ymd('2021-01-01
                                   '))
# test =  filter(marr_rf, date >= ymd('2020-01-01'))

model_table = model(train, aan = ETS(total ~ error('A') +
                                       trend('A') +
                                       season('N')))
model_table

fcst = forecast(model_table, h = 10)
fcst %>% autoplot(filter(marr_rf_fake, date > ymd('2017-01-01'))) +
  labs(title = "ETS(AAN)", y = "Number of weddings", x = NULL) +
  theme_light() + theme(legend.position = "none")
ggsave("../pictures/om_ts_02-052.png", dpi = 300, width = plot_width, height = plot_height, unit = 'cm')

# 2-76
nrow(marr_rf)
marr_rf_fake = mutate(marr_rf, total = total + 1 * (1:nrow(marr_rf)))
train = filter(marr_rf_fake, date < ymd('2020-01-01
                                   '))
# test =  filter(marr_rf, date >= ymd('2020-01-01'))

model_table = model(train, aaa = ETS(total ~ error('A') +
                                       trend('A') +
                                       season('A')))
model_table

fcst = forecast(model_table, h = 24)
fcst %>% autoplot(filter(marr_rf_fake, date > ymd('2016-01-01'))) +
  labs(title = "ETS(AAA)", y = "Number of weddings", x = NULL) +
  theme_light() + theme(legend.position = "none")
ggsave("../pictures/om_ts_02-076.png", dpi = 300, width = plot_width, height = plot_height, unit = 'cm')

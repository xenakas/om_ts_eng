...
autoplot(fcst, popnum) + theme_light() + labs(x = NULL, y = 'Population, millions', title = 'ETS(AAdN): population of Russia')
...
autoplot(aus, Electricity/1000) +
  labs(title = 'Volume of electricity produced in Australia',
       x = '', y = 'Volume, TWh') +
  theme_light()
...
autoplot(f_join, aus) +
  labs(title = 'ETS(MNM): Volume of electricity produced in Australia',
       x = '', y = 'Volume, TWh') +
